...
import com.rkgn.dto.GetUserForm;
import com.rkgn.dto.Result;
import com.rkgn.dto.UpdateForm;
...
public class UpdateFrame extends JFrame {
    ...
    private void okMouseClicked(MouseEvent e) {
        ...
        try (Socket socket = new Socket(Constants.SERVER_HOST, Constants.SERVER_PORT)) {
            UpdateForm updateForm = new UpdateForm(username, password, user.getRole());
            SocketUtils.send(socket, "update\n" + updateForm);
            Result result = SocketUtils.parseResult(socket);
            if (!result.getSuccess()) {
                Dialog.error(this, "修改角色错误：" + result.getErrorMsg());
                return;
            }
        } catch (IOException ex) {
            Dialog.error(this, "修改角色错误：" + ex.getMessage());
            return;
        }
        Dialog.info(this, "修改用户成功！");
        setVisible(false);
    }
    ...
    private void passwordFieldFocusGained(FocusEvent e) {
        String username = usernameField.getText();
        try (Socket socket = new Socket(Constants.SERVER_HOST, Constants.SERVER_PORT)) {
            GetUserForm getUserForm = new GetUserForm(username);
            SocketUtils.send(socket, "get user\n" + getUserForm);
            Result result = SocketUtils.parseResult(socket);
            if (!result.getSuccess()) {
                Dialog.error(this, "获取用户错误：" + result.getErrorMsg());
                return;
            }

            User user = null;
            if (result.getData() != null) {
                user = JSONObjectUtils.parseUser(result.getData());
            }

            if (user == null) {
                if (!hasGained) {
                    Dialog.error(this, "用户不存在！");
                    hasGained = true;
                    return;
                }
                hasGained = false;
            ...
            roleSelector.setSelectedItem(Role.translateMap.get(user.getRole()));
        } catch (IOException ex) {
            Dialog.error(this, "获取用户错误：" + ex.getMessage());
        }
    }

    private void usernameFieldInputMethodTextChanged(InputMethodEvent e) {
        if (!hasFlushed) {
            passwordField.setText("");
            roleSelector.setSelectedItem(null);
            hasFlushed = true;
    ...
    private void usernameFieldFocusLost(FocusEvent e) {
        hasFlushed = false;
    }
    ...
    private void initComponents() {
        ...
        //======== usernamePanelHolder ========
        {
            ...
            //======== usernamePanel ========
            {
                ...
                //---- usernameField ----
                usernameField.addInputMethodListener(new InputMethodListener() {
                    @Override
                    public void caretPositionChanged(InputMethodEvent e) {}
                    @Override
                    public void inputMethodTextChanged(InputMethodEvent e) {
                        usernameFieldInputMethodTextChanged(e);
                    }
                });
